...
from src.Yaku.Shuntsu import calculate, get_possible_combinations
...
class TestKoutsu(unittest.TestCase):
  ...
  # TODO:
  # see issue #1
  def test_shuntsu_with_quater_possibility(self):
    hand = [self.TileManager.getTile(t) for t in self.SAMPLE_TILES]
    sample_drawable_tiles = [self.TileManager.getTile(t) for t in self.SAMPLE_TILES]
  ...
  def test_shuntsu_with_single_hand_possibility(self):
  ...
  def test_shuntsu_with_character_tile(self):
  ...
  def test_shuntsu_with_no_drawable_tile(self):
    hand = [self.TileManager.getTile(t) for t in self.SAMPLE_TILES]
